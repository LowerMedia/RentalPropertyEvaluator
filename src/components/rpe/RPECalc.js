const RPECalc = {
	...
	// TODO: add calcs and results fields for IRR (internal rate of return)
	// TODO: add PITI (principal + interest + taxes + insurance)
	...
	EBDITA: (state) => {
		return state.calculated.CashFlowYearly + state.changeable.Taxes + state.calculated.TotalInterestPaidYearly + 0 + 0;// todo: add deprec and amort costs (amorit costs aren't applicable in RE... <- why?)
		//return totalIncomeYearly + totalTaxesPaidYearly + totalMortgageInterestPaidYearly + totalDeprectiationCostsYearly + totalAmoritizationCostsYearly 
	},
	...
	TotalInterestPaidMonthly: (state) => {
		return state.changeable.PurchasePrice * ( state.changeable.InterestRate * 0.00001 );
	},

	TotalInterestPaidYearly: (state) => {
		return 12 * ( state.changeable.PurchasePrice * ( state.changeable.InterestRate * 0.0001 ) );
	},
	...
	TotalExpensesYearly: (state) => { // NOT CORRECT NEED TO FIX
		return ( ( state.changeable.Insurance ) +  ( state.changeable.HOA ) +  ( state.changeable.OtherExpense ) +  ( state.changeable.Taxes ) +  ( state.calculated.TotalDollarExpensesEstimate * 12 ) ); // whole number int generated by percentage of income;
	},

	TotalExpensesMonthly: (state) => { // NOT CORRECT NEED TO FIX
